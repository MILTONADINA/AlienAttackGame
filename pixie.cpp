...
Pixie::Pixie(sf::Texture& texture, float x, float y, PixieType type) : pixieType(type) {
 ...
 */
void Pixie::move(float dx, float dy) {
    sprite.move(dx, dy);
}
 ...
 */
const sf::Sprite& Pixie::getSprite() const {
    return sprite;
}
